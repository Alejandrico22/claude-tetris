...
- Una **octava pieza especial**: un anillo 3×3 con hueco en el centro (`RING_CHANCE`, aparece con
  ~6 % de probabilidad frente a un 13 % de cada pieza estándar). Su hueco queda encerrado por sus
  propios bloques, así que al fijarse en el tablero esa celda vacía es **inalcanzable para siempre**:
  la fila que la contiene nunca se podrá completar ni eliminar.
...
- **Piezas**: definidas como matrices cuadradas. Para rotar se calcula la transposición + reverso de filas (`rotateCW`). `randomPiece()` sortea entre las 7 piezas estándar con `RING_CHANCE` de probabilidad extra para que salga el anillo (tipo 8).
